...
OpenGLViewer::OpenGLViewer(QWidget *parent, Model *model)
    : QOpenGLWidget{parent},
      ...
      connect(this, SIGNAL(changeColorSignal()), this, SLOT(changeBackgroundColor()));
      connect(this, SIGNAL(rotateModelSignalX(float)), this, SLOT(rotateModelX(float)));
      connect(this, SIGNAL(rotateModelSignalY(float)), this, SLOT(rotateModelY(float)));
      connect(this, SIGNAL(rotateModelSignalZ(float)), this, SLOT(rotateModelZ(float)));
      ...
      connect(this, SIGNAL(translateModeleSignalX(int)), this, SLOT(translateModeleX(int)));
      connect(this, SIGNAL(translateModeleSignalY(int)), this, SLOT(translateModeleY(int)));
      connect(this, SIGNAL(translateModeleSignalZ(int)), this, SLOT(translateModeleZ(int)));
}
...
/*virtual*/ void OpenGLViewer::resizeGL(int nWidth, int nHeight) {
  ...
  if(typeProj == 'c')projectMatrix.perspective(45, aspect, 0.1f, 800.0f);
  else if(typeProj == 'p')projectMatrix.ortho(-4.0f, 4.0f, -4.0f, 4.0f, 0.1f, 800.0f);
}
...
/*virtual*/ void OpenGLViewer::paintGL() {
  ...
  modelViewMatrix.translate(0.01*translateX, 0.01*translateY, -5.0 + 0.01*translateZ);
  modelViewMatrix.rotate(rotation);
  QMatrix4x4 scalingMatrix;
  scalingMatrix.scale(1.0f + 0.01*scaleX, 1.0f + 0.01*scaleY, 1.0f + 0.01*scaleZ);
...
void OpenGLViewer::setProj() {
  if(typeProj == 'c')typeProj = 'p';
  else if(typeProj == 'p')typeProj = 'c';
  update();
}
